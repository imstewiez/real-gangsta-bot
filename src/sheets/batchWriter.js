...
class BatchWriter {
  ...
  /** Flush — manda tudo em 1 chamada, com retry automático em erros transitórios.
   *
   * Backoff exponencial: 1s → 2s → 4s (capped 30s), max 3 tentativas.
   * Erros permanentes (4xx auth/config) falham imediatamente sem retry.
   */
  async flush() {
    if (!this.requests.length) return { replies: [] };
    this.validate();
    const requestsSnapshot = this.requests;
    let lastErr;
    for (let attempt = 0; attempt <= FLUSH_RETRY_MAX; attempt++) {
      try {
        const res = await this.sheets.spreadsheets.batchUpdate({
          spreadsheetId: this.spreadsheetId,
          requestBody: { requests: requestsSnapshot },
        });
        this.requests = [];
        return res.data;
      } catch (e) {
