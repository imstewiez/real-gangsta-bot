...
async function syncResumo(batch, sheetId) {
  ...
  const cmpData = [
    ...
    { label: 'Mortes', cur: current.deaths || 0, prev: previous?.deaths ?? null, fmt: NUM_FMT.INT },
    {
      ...
      prev: trend.supplied_units.previous,
      fmt: NUM_FMT.INT,
    },
    {
      label: 'Material Devolvido (un)',
      cur: trend.returned_units.current,
      prev: trend.returned_units.previous,
      fmt: NUM_FMT.INT,
    },
    {
      label: 'Material Perdido (un)',
      cur: trend.lost_units.current,
  ...
  row = kpiStrip(
    ...
    [
      ...
      },
      {
        ...
        value: totalOps,
        numberFormat: NUM_FMT.INT,
        delta: `média ${(totalOps / Math.max(diasOps, 1)).toFixed(1)}/dia útil`,
        deltaDirection: 'flat',
      },
      {
        label: 'Kills',
        value: totalKills,
        numberFormat: NUM_FMT.INT,
        delta: `média ${(totalKills / Math.max(diasOps, 1)).toFixed(1)}/dia útil`,
        deltaDirection: 'flat',
      },
      {
        label: 'Lucro Total',
  ...
  row = sectionHeader(batch, sheetId, row, {
    title: '📍 SPOTS · TOP 3 RENTÁVEIS',
    hint: 'por lucro líquido acumulado',
    columnCount: COL_COUNT,
  });
  ...
  row = rankingBlock(
    ...
    COL_COUNT,
    { labelCol: 1, valueCol: 4 }
  );
  row = spacer(batch, sheetId, row, COL_COUNT, 'SM');

  row = sectionHeader(batch, sheetId, row, {
    title: '⚠️ SPOTS · FLOP 3 PERIGOSOS',
    hint: 'por prejuízo ou mortes',
    columnCount: COL_COUNT,
  });
  ...
  row = sectionHeader(batch, sheetId, row, {
    ...
    hint: 'top 10 por eixo — 1º/2º/3º destacados',
    columnCount: COL_COUNT,
  });
