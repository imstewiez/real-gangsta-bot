...
async function saveWeeklyRanking(row) {
  const {
    ...
    operationsCount = 0, saidasCount = operationsCount, // compat
    weightedValue = 0, returnRate = 0,
  ...
  const res = await query(
    `INSERT INTO weekly_rankings
     ...
     RETURNING *`,
    [memberId, weekStart, weekEnd, deliveries, sales,
     saidasCount, weightedValue, returnRate,
