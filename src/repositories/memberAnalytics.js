...
async function getMaterialProfile(discordId) {
  ...
  const returnRate = fornecido.value > 0 ? (devolvido.value / fornecido.value) * 100 : 0;
  const lossRate = fornecido.value > 0 ? (perdido.value / fornecido.value) * 100 : 0;
...
async function getProfitProfile(discordId) {
  ...
  const stats = await query('SELECT profit_generated, saidas_total FROM member_saida_stats WHERE member_id = $1', [
    memberId,
  ]);
