...
async function getRankingPosition(discordId, weekStart) {
  ...
  return {
    ...
    above: above ? { displayName: above.display_name, discordId: above.discord_id, score: Number(above.score), position: Number(above.pos) } : null,
    below: below ? { displayName: below.display_name, discordId: below.discord_id, score: Number(below.score), position: Number(below.pos) } : null,
  };
}
...
async function getFullProfile(discordId) {
  ...
  const [weekStats, monthStats, allTimeStats, weekRank, monthRank, allTimeRank, evolution, streak, saidaStats, allTimeRow] = await Promise.all([
