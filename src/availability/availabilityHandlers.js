...
async function handleVoteSelect(interaction) {
  ...
  if (!value) return safeReply(interaction, { content: `${EMOJI.WARN} Sem opção escolhida.`, flags: MessageFlags.Ephemeral }, { dismissible: true });
...
async function handleVoteAll(interaction) {
  ...
  if (!result.ok) {
    return safeReply(interaction, { content: `${EMOJI.WARN} Não foi possível registar.`, flags: MessageFlags.Ephemeral }, { dismissible: true });
  }
  const m = stateMeta(state);
  return safeReply(interaction, {
...
async function handleSummary(interaction) {
  ...
  if (!text) return safeReply(interaction, { content: ERRORS.SESSION_NOT_FOUND(), flags: MessageFlags.Ephemeral }, { dismissible: true });
...
async function handleRefresh(interaction) {
  ...
  try {
    ...
    return safeReply(interaction, { content: `${EMOJI.REFRESH} Actualizado.`, flags: MessageFlags.Ephemeral }, { dismissible: true });
  } catch (e) {
    warn(`[AVAIL] refresh falhou: ${e.message}`);
    return safeReply(interaction, { content: `${EMOJI.WARN} Não foi possível actualizar.`, flags: MessageFlags.Ephemeral }, { dismissible: true });
  }
}
