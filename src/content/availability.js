...
const AVAILABILITY = {
  ...
  SESSION_OPENED: (id, channelId) =>
    `${E.OK} Sessão **#${id}** publicada em <#${channelId}>.`,
  SESSION_ALREADY_OPEN: (id) =>
    `${E.WARN} Já há sessão aberta hoje (**#${id}**). Fecha primeiro.`,
  SESSION_CLOSED: (id) =>
    `${E.BLOQUEADO} Sessão **#${id}** fechada.`,
  VOTE_RECORDED: (slot, label, emoji) =>
    `${emoji} **${slot}** → **${label}**`,
  VOTE_BULK_RECORDED: (n, label, emoji) =>
    `${emoji} ${n} slots marcados como **${label}**.`,
  REASON: {
