...
async function handleCreateSaidaButton(interaction) {
  if (!isChefia(interaction.member)) {
    return safeReply(interaction, { content: MESSAGES.NO_PERMISSION('criar saídas'), flags: MessageFlags.Ephemeral }, { dismissible: true });
  }
  const modal = new ModalBuilder()
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('type').setLabel('Tipo de saída')
...
async function handleCreateSaidaModal(interaction) {
  ...
  if (!SAIDA_TYPES.includes(type)) {
    return safeReply(interaction, { content: `Tipo inválido. Usa: ${SAIDA_TYPES.join(', ')}` }, { dismissible: true });
  }
  try {
    ...
    const embed = successEmbed('Saída Criada', `Saída **#${s.id}** — ${type}\nData: ${date}${spot ? `\nSpot: ${spot}` : ''}`);
...
async function handleCloseSaidaButton(interaction) {
  if (!isChefia(interaction.member)) {
    return safeReply(interaction, { content: MESSAGES.NO_PERMISSION('fechar saídas'), flags: MessageFlags.Ephemeral }, { dismissible: true });
  }
  const open = await saidaRepo.findOpen();
  if (!open.length) {
    return safeReply(interaction, { content: 'Sem saídas abertas.', flags: MessageFlags.Ephemeral }, { dismissible: true });
  }
  ...
  await safeReply(interaction, { content: 'Escolhe a saída a fechar:', components: [row], flags: MessageFlags.Ephemeral });
}
...
async function handleMarkDeadSelect(interaction) {
  ...
  if (!deadIds.length) {
    pendingSaidaContext.delete(interaction.user.id);
    return safeReply(interaction, { content: `Saída #${saidaId} — nenhum morto marcado.`, flags: MessageFlags.Ephemeral }, { dismissible: true });
  }
  const report = [];
  for (const discordId of deadIds) {
    try {
      ...
      report.push(`☠️ <@${discordId}> — ${diedWithItems.length ? `${diedWithItems.length} item(s) → perda` : 'sem material'}`);
    } catch (e) {
...
async function handleViewSaidasButton(interaction) {
  ...
  if (!list.length) return safeReply(interaction, { content: 'Sem saídas registadas.' }, { dismissible: true });
  const statusEmoji = { aberta: '🟢', em_preparacao: '🟡', em_curso: '🟠', concluida: '✅', cancelada: '❌' };
  const resultEmoji = { vitoria: '🏆', derrota: '💀', empate: '🤝', sem_conflito: '🕊️', abortada: '🚫' };
  ...
  const embed = brandEmbed().setTitle('Saídas Recentes').setDescription(lines.join('\n'));
  return safeReply(interaction, { embeds: [embed] }, { dismissible: true });
}
...
async function handleAddParticipantButton(interaction) {
  ...
  const open = await saidaRepo.findOpen();
  if (!open.length) return safeReply(interaction, { content: 'Sem saídas abertas.', flags: MessageFlags.Ephemeral }, { dismissible: true });
  const options = open.map(s => ({
    label: `#${s.id} — ${s.operation_type} (${s.date})`.slice(0, 100),
    description: s.spot || 'Sem spot',
    value: String(s.id),
  ...
  const row = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder().setCustomId('saida::select_add_participant')
      ...
      .addOptions(options.slice(0, 25)));
  await safeReply(interaction, { content: 'Em que saída vais adicionar participantes?', components: [row], flags: MessageFlags.Ephemeral });
}
...
async function handleAddParticipantSelect(interaction) {
  ...
  await safeUpdate(interaction, {
    content: `Saída **#${saidaId}** — selecciona participantes:`,
    components: [userMenu],
  });
}
...
async function handleParticipantUsersSelect(interaction) {
  ...
  const parts = interaction.customId.split('::');
  const saidaId = parseInt(parts[2]);
  if (!saidaId) return safeReply(interaction, { content: 'Saída inválida.', flags: MessageFlags.Ephemeral }, { dismissible: true });
  ...
  const lines = [];
  if (added.length) {
    lines.push(`**${added.length}** participante(s) na Saída **#${saidaId}**:`, added.map(u => `<@${u}>`).join(', '));
  }
  if (errors.length) { lines.push('', '**Falhas:**', ...errors); }
  return safeReply(interaction, { embeds: [successEmbed('Participantes Adicionados', lines.join('\n'))], flags: MessageFlags.Ephemeral }, { dismissible: true });
}
...
async function handleRegisterMaterialButton(interaction) {
  if (!isChefia(interaction.member)) {
    return safeReply(interaction, { content: MESSAGES.NO_PERMISSION('registar material em saídas'), flags: MessageFlags.Ephemeral }, { dismissible: true });
  }
  const open = await saidaRepo.findOpen();
  if (!open.length) return safeReply(interaction, { content: 'Sem saídas abertas.', flags: MessageFlags.Ephemeral }, { dismissible: true });
  const options = open.map(s => ({
  ...
  const row = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder().setCustomId('saida::select_material_op')
      .setPlaceholder('Qual saída?').setMinValues(1).setMaxValues(1)
      .addOptions(options.slice(0, 25)));
  await safeReply(interaction, { content: 'Material para que saída?', components: [row], flags: MessageFlags.Ephemeral });
}
...
async function handleMaterialOpSelect(interaction) {
  ...
  const row = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder().setCustomId('saida::select_material_direction')
      .setPlaceholder('Tipo de movimento').setMinValues(1).setMaxValues(1)
      .addOptions(directionOptions));
  await safeUpdate(interaction, { content: `Saída **#${saidaId}** — tipo de registo:`, components: [row] });
}
...
async function handleMaterialDirectionSelect(interaction) {
  ...
  const menu = await buildItemSelectMenu('saida::select_material_item', 'Seleciona o material');
...
async function handleMaterialQtyModal(interaction) {
  ...
  try {
    ...
    const embed = successEmbed('Material Registado',
      `**${quantity}x** ${item?.name || 'Item'} — ${dirLabels[ctx.direction]}\nSaída **#${ctx.saidaId}**${notes ? `\nNotas: ${notes}` : ''}`
    );
    return safeReply(interaction, { embeds: [embed] }, { dismissible: true });
  } catch (e) {
    return safeReply(interaction, { content: `Erro: ${e.message}` }, { dismissible: true });
...
async function handleIssueToParticipantButton(interaction) {
  ...
  const row = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder().setCustomId('saida::issue_select_saida')
      .setPlaceholder('Qual saída?').setMinValues(1).setMaxValues(1)
      .addOptions(options.slice(0, 25)));
  await safeReply(interaction, { content: '🎯 Fornecer material nominal — qual saída?', components: [row], flags: MessageFlags.Ephemeral });
}
...
async function handleIssueSaidaSelect(interaction) {
  ...
  if (!participants.length) {
    pendingSaidaContext.delete(interaction.user.id);
    return safeUpdate(interaction, { content: `Saída **#${saidaId}** sem participantes ainda. Adiciona primeiro em "Participantes".`, components: [] });
  }
  ...
  const row = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder().setCustomId('saida::issue_select_participant')
      .setPlaceholder('A quem vai o material?').setMinValues(1).setMaxValues(1)
      .addOptions(options));
  await safeUpdate(interaction, { content: `Saída **#${saidaId}** — participante:`, components: [row] });
}
...
async function handleIssueParticipantSelect(interaction) {
  ...
  const ctx = pendingSaidaContext.get(interaction.user.id);
  if (!ctx || ctx.action !== 'issue_to_participant') return safeReply(interaction, { content: 'Sessão expirada.', flags: MessageFlags.Ephemeral }, { dismissible: true });
...
async function handleIssueQtyModal(interaction) {
  ...
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  const ctx = pendingSaidaContext.get(interaction.user.id);
  if (!ctx || ctx.action !== 'issue_to_participant') return safeReply(interaction, { content: 'Sessão expirada.' }, { dismissible: true });
